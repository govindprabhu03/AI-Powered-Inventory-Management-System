...
export const metadata = { title: "Sales orders · Smart Inventory" };

export default async function SalesPage() {
  const ctx = await requireContext();
  const canCreate = canManageSales(ctx.activeOrg.role);

  const supabase = await createClient();
  const { data: orders } = await supabase
    .from("sales_orders")
    .select("id, order_number, status, payment_status, created_at, customers(name)")
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-8 py-10">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Selling stock to customers.</p>
        </div>
        {canCreate && (
          <Button nativeButton={false} render={<Link href="/sales/new">New sales order</Link>} />
        )}
      </div>
      ...
      {orders && orders.length > 0 ? (
              ...
              {orders.map((o) => (
                <TableRow key={o.id}>
                  <TableCell className="font-medium">
                    <Link href={`/sales/${o.id}`} className="underline-offset-4 hover:underline">
                      {o.order_number}
                    </Link>
                  </TableCell>
        ...
        <div className="grid gap-3 rounded-lg border border-dashed px-6 py-16 text-center">
          ...
          {canCreate && (
            <div>
              <Button nativeButton={false} render={<Link href="/sales/new">Create your first sales order</Link>} />
            </div>
          )}
        </div>
      )}
